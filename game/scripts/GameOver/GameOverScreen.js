...
class GameOverScreen {
    drawGameOverScreen() {
        ...
        ctx.fillText("CurrentScore: " + manageScore.currentScore, gameArea.canvas.width / 2 - gameArea.canvas.width / 6, gameArea.canvas.height / 2 + gameArea.canvas.height / 7);
        ctx.fillStyle = "green";
        ctx.strokeStyle = "white";
        this.roundRect(ctx, gameArea.canvas.width / 2 - 225, gameArea.canvas.height - 300, 450, 200, 50, true);
        this.roundRect(ctx, gameArea.canvas.width / 2 - 225, gameArea.canvas.height - 300, 450, 200, 50, false);
    ...
    newGameButtonClicked() {
        ...
        document.onclick = function (event) {
            posX = event.clientX;
            posY = event.clientY;
            if (this.checkClick(posX, posY, gameArea.canvas.width / 2 - 225, gameArea.canvas.height - 300, 450, 200)) {
                document.location.reload();
            }
        }
    }

    checkClick(pX, pY, rX, rY, w, h) {
